...
io.on('connection', (socket) => {
  ...
  socket.on('chat:message', async (dataS) => {
    ...
    if (dataS.type === 'deal-resolve') {
      ...
      if (dataS.data.status) {
        ...
        await new Appointment({seller: chat.seller, chat: chat._id, buyer: chat.buyer, ad: chat.ad, date: new Date(appointmentDate.setHours(appointmentDate.getHours() + 2)), pendingTokens: chat.price, location: {type: 'Point', coordinates: chat.buyer.location.coordinates}}).save();
  ...
  socket.on('call:new-call', async (data) => {
    ...
    if (connectedSockets && connectedSockets > 1) {
  ...
  socket.on('call:decline-call', async (data) => {
    socket.in(data.chatId).emit('call:declined-call', data);
  });

  socket.on('call:accept-call', async (data) => {
    socket.in(data.chatId).emit('call:accepted-call', data);
  });

  socket.on('call:handUp-call', async (data) => {
    socket.in(data.chatId).emit('call:finished', data);
  });

  socket.on('call:handShake', async (data) =>{
    socket.in(data.chatId).emit('call:handShakeRequest', data);
  });

  socket.on('call:handShakeAccept', async (data) => {
    socket.in(data.chatId).emit('call:handShakeRequestAccepted', data);
  });

  socket.on('call:handUpDestroyPeer', async (data) => {
