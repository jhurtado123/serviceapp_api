  ...
  socket.on('chat:message', async (dataS) => {
    ...
    if (dataS.type === 'deal-resolve') {
      ...
      if (dataS.data.status) {
        ...
        await new Appointment({seller: chat.seller, buyer: chat.buyer, ad: chat.ad, date: dataS.date, pendingTokens: chat.price, location: {type: 'Point', coordinates: chat.buyer.location.coordinates}}).save();
